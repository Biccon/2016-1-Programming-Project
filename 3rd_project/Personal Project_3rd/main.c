...
void save_in_ascii(LinkedList *lists){
    FILE *fp = fopen("myDic.txt", "w+");
    char *temp = (char*)malloc(sizeof(char) * 100);
    int i;
    for(i=0;i<SIZE;i++){
        int nodeSize = get_size(lists + i);
        int k;
        strcpy(temp, "");
        for(k=0;k<nodeSize;k++){
            Node * tempNode = get_node_by_index(lists + i, k);
            strcat(temp, tempNode -> data);
            if(k != nodeSize - 1)
                strcat(temp, " ");
        }
        fprintf(fp, "%s\n", temp);
...
void save_in_binary(LinkedList *lists){
    boolean chkEmpty = true;
    FILE *fp = fopen("myDic.bin", "wb+");
    int i;
    for(i=0;i<SIZE;i++){
        int k;
        for(k=0;k<get_size(lists + i);k++){
            chkEmpty = false;
            Node *temp = get_node_by_index(lists + i, k);
...
void read_in_binary(LinkedList *lists){
    FILE *fp = fopen("myDic.bin", "rb+");
    if(fp == NULL)
        return;
    Node *temp = (Node*)malloc(sizeof(Node));
    while(fread(temp, sizeof(Node), 1, fp)){
        insert_node(lists, temp -> data);
    }
    fclose(fp);
}
...
int get_menu(){
    ...
    printf("[3] Delete [4] Save in ASCII\n");
    printf("[5] Save in BINARY [6] Quit\n");
    printf(": ");
    scanf("%d", &menu);
    return menu;
}
...
int main(void){
    char input[30];
    Offset *tempOffset = NULL;
    Node *tempNode;
    int menu;
    LinkedList *list = (LinkedList *)malloc(sizeof(LinkedList) * SIZE);
    init_list(list);
    read_in_binary(list);
    while((menu = get_menu()) != 6){
        int count;
        if(menu == 1){
            printf("찾을 단어를 입력하세요 : ");
            scanf("%s", input);
            ...
            printf("INSERT: %d %d @ %d\n", tempOffset -> row, tempOffset -> col, count);        } else if(menu == 3){
